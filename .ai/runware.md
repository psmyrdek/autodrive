...
Introduction
Inpainting is a powerful image editing technique that allows you to selectively modify specific areas of an image while preserving the rest intact. Unlike standard image-to-image which transforms the entire image, inpainting focuses only on masked regions, making it ideal for targeted edits like object removal, replacement, or enhancement.

Colorful landscape with a lake, flowers, and butterfly-shaped cutouts
...
Cozy bedroom with a neatly made bed featuring white and beige bedding, flanked by wooden nightstands with lamps, a framed picture above the headboard, light curtains over a window, and a soft rug on a wooden floor
Prompt: bedroom
Black and white mask image
CMask
Cozy bedroom with a neatly made bed featuring white and beige bedding, flanked by wooden nightstands with lamps, light curtains over a window, and a soft rug on a wooden floor
Prompt: wall
...
Key parameters
Seed and mask image: The foundation
...
Opening your original image in your editor of choice.
Creating a new layer that will serve as your mask.
Using painting tools with white color to mark areas you want to modify.
Filling the rest of the layer with black to indicate areas to preserve.
Optionally using blur or gradient tools to create smooth transitions at boundaries.
Exporting just the mask layer as a separate grayscale image.
For simple masks, even basic applications can work well. The important part is creating a clear distinction between areas to modify (white) and areas to preserve (black). Once created, you can upload both your original image and the mask to Runware using the Image Upload API to use them in inpainting requests.

If you prefer an automated approach, check out our automatic mask generation section below for tools that can create masks for you based on detected objects, faces, or other elements.

Strength: Controlling transformation intensity
...
A chubby raccoon holding a sparkly balloon shaped like a star, standing in front of a night sky filled with fireworks
A chubby raccoon holding a sparkly balloon shaped like a star, standing in front of a night sky filled with fireworks
...
Cropping the original image to include just the masked area plus the specified margin in pixels.
Performing inpainting at a higher effective resolution on this cropped region.
Seamlessly blending the enhanced region back into the original image.
...
A woman standing in a field, looking at the camera from a distance, her dress blowing in the wind under a soft, overcast sky
Original image
Woman face
...
The maskMargin parameter is particularly powerful when combined with our automatic mask generation feature. For example, when using face detection to create a mask, adding a maskMargin allows the model to enhance facial features with exceptional detail.
...
Model: Specialized inpainting models
...
FLUX Fill (runware:102@1, part of FLUX Tools) is FLUX's dedicated inpainting model that uses a different technical approach than traditional models. Unlike standard models that require manual parameter tuning, FLUX Fill automatically determines optimal settings based on the input image and mask, providing less granular control but often producing better results for complex scenes.
A smiling axolotl in a cozy underwater reading nook, surrounded by coral bookshelves and glowing jellyfish lamps
Original image
Black and white mask image
Mask
...
A smiling axolotl in a cozy underwater reading nook, surrounded by coral bookshelves and glowing jellyfish lamps, an astronaut knocking on window
A smiling axolotl in a cozy underwater reading nook, surrounded by coral bookshelves and glowing jellyfish lamps, an astronaut knocking on window
...
Our Image Masking API can automatically detect and create masks for specific elements in your images, making the inpainting workflow more efficient and precise.
...
A man in scorched robes raises his hand, and shards of obsidian float around him in a slow orbit, each one crackling with red lightning
Original image
Black and white mask image
Full-person mask
...
A juicy cheeseburger with bacon, fries on a wooden board, next to a cold can of Zuno Cola, soft natural lighting
Original
Black and white mask
Mask
A juicy cheeseburger with bacon, fries on a wooden board, soft natural lighting
Inpainted
...
For efficient workflow, you can use our Image Masking API with automatic detection and then invert the mask to target only the background. You can also use any third-party segmentation tools that separate foreground from background automatically.
...
A high-quality product photo of a matte black wireless headphone set, centered composition, soft shadows, white background with slight depth, subtle gradient lighting for a professional look
Original
Black and white mask
Mask
A high-quality product photo of a matte black wireless headphone set, centered composition, soft shadows, on a wood desktop with plants and blue neon lights on the wall, product photography, subtle gradient lighting for a professional look
Inpainted
Recommended settings:
...
High CFGScale.
High steps.
Multi-stage inpainting is very useful here.
...
A full-body photo of a woman wearing a plain white t-shirt and jeans, standing outdoors in soft daylight, looking at the camera
Original
Black and white mask
Mask
A full-body photo of a woman wearing an elegant dark blue dress with a golden belt, standing outdoors in soft daylight, looking at the camera
Inpainted
Recommended settings:
...
A modern living room with a sofa and coffee table, clean and minimal layout, sunlight coming in from the side
Original
Black and white mask
Mask
A modern living room with a sofa and coffee table, The Great Wave Off Kanagawa painting with a black frame, clean and minimal layout, sunlight coming in from the side
Inpainted
Recommended settings:
...
When you've accidentally included important elements in your mask that you want to preserve.
When using maskMargin, which limits the model's ability to see the broader context.
