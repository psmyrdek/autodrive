...
export default function TrackBuilder() {
  ...
  const handleLoadTrack = async (name: string) => {
    ...
    if (track) {
      ...
      if (track.texture) {
        textureHandlers.setTexture(track.texture);
      }
    }
  };
  ...
  return (
    <div className='relative h-full min-h-fit bg-gray-900 overflow-auto'>
      ...
      <div className='absolute inset-0 flex items-center justify-center'>
        ...
        {textureState.isGenerating && (
          <div className='absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm'>
            <div className='bg-gradient-to-br from-teal-600 via-cyan-700 to-indigo-700 rounded-2xl shadow-2xl p-8'>
              <div className='flex flex-col items-center gap-6'>
                ...
                <div className='flex gap-2'>
                  <div className='w-3 h-3 bg-white/60 rounded-full animate-bounce [animation-delay:0ms]'></div>
                  <div className='w-3 h-3 bg-white/60 rounded-full animate-bounce [animation-delay:150ms]'></div>
                  <div className='w-3 h-3 bg-white/60 rounded-full animate-bounce [animation-delay:300ms]'></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className='absolute top-6 left-1/2 -translate-x-1/2 z-10'>
        <div className='bg-gray-800/95 backdrop-blur-sm rounded-lg shadow-xl border border-gray-700 p-3 flex items-center gap-3'>
          <input
            type='text'
            value={trackName}
            onChange={(e) => setTrackName(e.target.value)}
            className='text-lg font-semibold text-gray-200 bg-transparent border-none outline-none focus:ring-0 min-w-[300px] text-center placeholder-gray-500'
      ...
      <div className='absolute bottom-6 left-1/2 -translate-x-1/2 z-10'>
        <div className='bg-gray-800/95 backdrop-blur-sm rounded-lg shadow-xl border border-gray-700 p-3'>
          <div className='flex items-center gap-2'>
            ...
            <button
              onClick={handleClear}
              className='p-3 text-gray-300 rounded-lg hover:bg-gray-600 font-medium transition-all flex items-center gap-2 border border-gray-600'
              title='Clear Canvas'
            >
