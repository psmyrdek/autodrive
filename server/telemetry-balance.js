...
function analyzeTelemetryBalance() {
  ...
  const files = fs.readdirSync(TELEMETRY_DIR).filter(file => file.endsWith('.json'));
  ...
  const totalSamples = Object.values(counts).length > 0 ? files.reduce((sum, file) => {
  ...
  // Display results
  console.log('=== OVERALL KEY PRESS STATISTICS ===');
  ...
  console.log(`W (Accelerate):    ${counts.w.toLocaleString().padStart(10)} presses  (${percentages.w}%)`);
  console.log(`A (Turn Left):     ${counts.a.toLocaleString().padStart(10)} presses  (${percentages.a}%)`);
  console.log(`S (Brake/Reverse): ${counts.s.toLocaleString().padStart(10)} presses  (${percentages.s}%)`);
  console.log(`D (Turn Right):    ${counts.d.toLocaleString().padStart(10)} presses  (${percentages.d}%)`);
  console.log();
  ...
  if (accelSteerTotal > 0) {
    ...
    const steerPercent = ((counts.a + counts.d) / accelSteerTotal * 100).toFixed(2);
